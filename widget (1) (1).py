...
import random
import sqlite3
link="C:/Users/Dell/OneDrive/Desktop/python_check/"
...
#new registration===========================================================================
    def clicked3():
            ...
            """selected = IntVar()
            rad1 = Radiobutton(windows,text='MALE', value="Male", variable=selected)
            rad2 = Radiobutton(windows,text='FEMALE', value="Female", variable=selected)
            def clicked():
                btn1=selected.get()
            btn = Button(windows, text="Click Me", command=clicked)
            rad1.grid(column=2, row=5,padx=10,pady=10)
            rad2.grid(column=3, row=5,padx=10,pady=10)
            btn.grid(column=4, row=5,padx=10,pady=10)"""
    ...
    #if len(ent6.get())==0:
        #messagebox.showwarning('Message title', 'PLEASE ENTER A VALID ID')
...
#DATABASE================================================================================================
                ...
                link="C:/Users/Dell/Desktop/python_project_test/"
                db=sqlite3.connect(link+'studentdatabaseinfo1.db')
                cursor=db.cursor()
                e1=ent.get()
                e2=ent2.get()
               ...
               # roll_no()
                ...
                #val=(ent.get(),ent2.get(),ent3.get())
                #sql=" INSERT INTO INFO (NAME,FATHER,MOTHER,DOB) VALUES (?, ?, ?, ?)"
               # db.execute(sql,val);
    ...
    def clicked2():
        # First first the content from the table in Row
...
#admin pages
    def clicked1():
        ...
        if uid.get()=="a" and pss.get()=="1":
            ...
            gn.grid(column=2,row=5,padx=10,pady=10)
            """selected = IntVar()
            rad1 = Radiobutton(windows,text='MALE', value=1, variable=selected)
            rad2 = Radiobutton(windows,text='FEMALE', value=2, variable=selected)
            def clicked():
                btn1=selected.get()
            btn = Button(windows, text="Click Me", command=clicked)
            rad1.grid(column=2, row=5,padx=10,pady=10)
        ...
        #messagebox.showwarning('Message title', 'PLEASE ENTER A VALID ID')
            ...
            #btn4=Button(windows,text="SEARCH",command=clicked1)
            #btn4.grid(column=3,row=19)

            btn5=Button(windows,text="DISPLAY",command=clicked1)
            btn5.grid(column=0,row=22)

            
            #btn6=Button(windows,text="CLEAR",command=clicked1)
            #btn6.grid(column=5,row=19)

            btn7=Button(windows,text="EXIT",command=uidpasschk)
            btn7.grid(column=0,row=23)

            #btn8=Button(windows,text="CLEAR",command=clicked1)
            #btn8.grid(column=7,row=19)
          ...
          #  def submitted():
                #windows.destroy()
                #w3()
            #btn=Button(windows,text="SUBMIT",command=submitted)
